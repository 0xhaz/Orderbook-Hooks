...
import {IOrderbook, ExchangeOrderbook} from "./IOrderbook.sol";
...
contract MatchingEngine is Initializable, ReentrancyGuard, AccessControl {
    ...
    bytes32 private LISTING_COORDINATOR_ROLE = keccak256("LISTING_COORDINATOR_ROLE");
    // Market maker role
    bytes32 private MARKET_MAKER_ROLE = keccak256("MARKET_MAKER_ROLE");
    // fee recipient for point storage
    address private feeTo;
    // fee denominator representing 0.001%, 1/1_000_000 = 0.0001%
    uint32 public immutable feeDenom = 1_000_000;
    // Factories
    ...
    struct OrderData {
        ...
        address orderbook;
        /// Head pricce on bid orderbook, the highest bid price
        uint256 bidHead;
        /// Head price on ask orderbook, the lowest ask price
        uint256 askHead;
        /// Market price on pair
        uint256 mp;
        /// Spread(volatility) limit on limit/market | buy/sell for market suspension(e.g circuit breaker, tick)
        uint32 spreadLimit;
        /// Make order id
        uint32 makeId;
     ...
     * @param owner The address of the owner whose order is matched with the sender
     * @param price the price at which the order is matched
     * @param amount the matched amount of the asset being traded in the match. if isBid == false, it is base assets, if isBid == true, it is quote assets
     * @param clear whether the order deposit has been cleared
     */
    event OrderMatched(
     ...
     * @param orderbookFactory_ The address of the orderbook factory
     * @param feeTo_ The address of the fee recipient
     * @param WETH_ The address of the WETH contract
     *
     * Requirements:
     * - `msg.sender` must have the default admin role
     */
    function initialize(address orderbookFactory_, address feeTo_, address WETH_) external initializer {
     ...
     * buys the ase asset using the quote asset at the best available price in the order up to `n` orders
     ...
     * @param quoteAmount The amount of the quote asset to be used for the market buy order
     * @param isMaker Boolean indicating if a order should be made at the market price in ordebook
     ...
     * @return placed placed amount of the base asset
     * @return id order id
     */
    function marketBuy(
        address base,
        ...
        (orderData.withoutFee, orderData.bidHead, orderData.askHead) = _limitOrder(
            ...
            recipient,
            true,
            (orderData.mp * (10_000 + orderData.spreadLimit)) / 10_000,
            n
        );
        ...
        // add make order on market price, reuse orderData. Is for storing placed order id
     ...
     * sells the base asset for the quote asset at the best available price in the order up to `n` orders
     * and make an order at the market price
     * @param orderbook  The address of the orderbook contract
     * @param bidHead  The highest bid price
     * @param askHead  The lowest ask price
        ...
        returns (uint256 price)
    {
        ...
        if (askHead == 0 && bidHead == 0) {
            // lmp must exists unless there is no order in the orderbook
            if (lmp != 0) {
                up = (lmp * (10_000 + spread)) / 10_000;
                return up;
            }
        } else if (askHead == 0 && bidHead != 0) {
            if (lmp != 0) {
                uint256 temp = (bidHead >= lmp ? bidHead : lmp);
                up = (temp * (10_000 + spread)) / 10_000;
                return up;
            }
            up = (bidHead * (10_000 + spread)) / 10_000;
            return up;
        } else if (askHead != 0 && bidHead == 0) {
            if (lmp != 0) {
                up = (lmp * (10_000 + spread)) / 10_000;
     ...
     * sells the base asset for the quote asset at the best available price in the orderbooko up to `n` orders
     * and make an order at the market price
     * @param base The address of the base asset for the trading pair
     * @param quote The address of the quote asset for the trading pair
     ...
     * @param n The maximum number of orders to match in the orderbook
     * @return makePrice price where the order is placed
     * @return placed placed amount of the quote asset
     * @return id order id
     */
    function marketSell(
        address base,
        ...
        // reuse baseAmount for storing amount after taking fees
        ...
        (orderData.withoutFee, orderData.bidHead, orderData.askHead) = _limitOrder(
            ...
            recipient,
            false,
            (orderData.mp * (10_000 + orderData.spreadLimit)) / 10_000, // spread limit
            n
        );
        ...
        returns (uint256 price)
    {
        ...
        if (askHead == 0 && bidHead == 0) {
            ...
            if (lmp != 0) {
                ...
                down = (temp * (10_000 - spread)) / 10_000;
                return down == 0 ? 1 : down;
            }
            down = (askHead * (10_000 - spread)) / 10_000;
            return down == 0 ? 1 : down;
        } else {
            if (lmp != 0) {
                uint256 temp = lmp <= askHead ? lmp : askHead;
     ...
     * @return id order id
     */
    function marketBuyETH(address base, bool isMaker, uint32 n, uint32 uid, address recipient)
        external
        payable
     ...
     * @return id order id
     */
    function marketSellETH(address quote, bool isMaker, uint32 n, uint32 uid, address recipient)
        external
        payable
        returns (uint256 makePrice, uint256 placed, uint32 id)
    {
    ...
    /**
     * @dev Executes a limit buy order, with spread limit of 1% for price actions
     ...
     * @param base The address of the base asset for the trading pair
     * @param quote The address of the quote asset for the trading pair
     * @param price The price, base/quote regardless of decimals of the assets in the pair represented with 8 decimals (if 1_000, base is 1_000x quote)
     * @param quoteAmount The amount of the quote asset to be used for the limit buy order
     * @param isMaker Boolean indicating if a order should be made at the limit price in ordebook
     * @param n The maximum number of orders to match in the orderbook
     * @param recipient The address of the recipient to receive traded asset and claim ownership of made order
     * @return makePrice price where the order is placed
     * @return placed placed amount of the base asset
        ...
        returns (uint256 price)
    {
        ...
        if (askHead == 0 && bidHead == 0) {
            ...
            up = (askHead * (10_000 + spread)) / 10_000;
            up = lp >= up ? up : lp;
            return up >= askHead ? askHead : up;
        } else {
            up = (bidHead * (10_000 + spread)) / 10_000;
            // First, set upper limit on make price for market suspension
            up = lp >= up ? up : lp;
            // upper limit on make price must not go above the lowest ask price
            return up >= askHead ? askHead : up;
        }
    }
     ...
     * @param quote The address of the quote asset for the trading pair
     * @param price The price, base/quote regardless of decimals of the assets in the pair represented with 8 decimals (if 1_000, base is 1_000x quote)
     * @param baseAmount The amount of the base asset to be sold in the limit sell order
     * @param isMaker Boolean indicating if a order should be made at the limit price in ordebook
     * @param n The maximum number of orders to match in the orderbook
     * @return makePrice price where the order is placed
     * @return placed placed amount of the quote asset
        ...
        returns (uint256 price)
    {
        ...
        if (askHead == 0 && bidHead == 0) {
            ...
            down = (bidHead * (10_000 - spread)) / 10_000;
            return lp <= down ? down : lp;
        } else if (askHead != 0 && bidHead == 0) {
            down = (askHead * (10_000 - spread)) / 10_000;
            ...
            down = (bidHead * (10_000 - spread)) / 10_000;
            // First, set lower limit on make price for market suspension
            down = lp <= down ? down : lp;
            // lower limit on make price must not go below the highest bid price
            return down <= bidHead ? bidHead : down;
        }
    }
     ...
     */
    function limitBuyETH(address base, uint256 price, bool isMaker, uint32 n, uint32 uid, address recipient)
        external
        payable
        returns (uint256 makePrice, uint256 placed, uint32 id)
    {
     ...
     * @return id placed order id
     */
    function limitSellETH(address quote, uint256 price, bool isMaker, uint32 n, uint32 uid, address recipient)
        external
        payable
        returns (uint256 makePrice, uint256 placed, uint32 id)
    {
    ...
    /**
     * @dev Creates an orderbook for a new tradking pair and returns its address
     * @param base The address of the base asset for the trading pair
     * @param quote The address of the quote asset for the trading pair
     * @param initMarketPrice The initial market price for the trading pair
     * @return book The address of the orderbook contract
     */
     ...
     * @param base The address of the base asset for the trading pair
     * @param quote The address of the quote asset for the trading pair
     * @param isBid Boolean indicating whether the order to cancel is an ask order
        ...
        returns (uint256 refunded)
    {
        ...
        uint256 remaining = IOrderbook(orderbook).cancelOrder(isBid, orderId, msg.sender);
    ...
    /**
     * @dev Returns the base and quote assets for the given orderbook
    ...
    /**
     * @dev Return addresses of pairs in orderbookFactory registry
    ...
    /**
     * @dev returns addresses of pairs in OrderbookFactory registry
     * @return mktPrices list of market prices from start to end
     */
    function getMktPrices(uint256 start, uint256 end) external view returns (uint256[] memory mktPrices) {
        IOrderbookFactory.Pair[] memory pairs = IOrderbookFactory(orderbookFactory).getPairs(start, end);
        mktPrices = new uint256[](pairs.length);
        for (uint256 i; i < pairs.length; i++) {
            try this.mktPrice(pairs[i].base, pairs[i].quote) returns (uint256 price) {
                uint256 p = price;
                mktPrices[i] = p;
            } catch {
                uint256 p = 0;
    ...
    function getMktPricesWithIds(uint256[] memory ids) external view returns (uint256[] memory mktPrices) {
        IOrderbookFactory.Pair[] memory pairs = IOrderbookFactory(orderbookFactory).getPairsWithIds(ids);
        mktPrices = new uint256[](pairs.length);
     ...
     * @param price The price to retrieve orders for
     ...
     * @param quote The address of the quote asset for the trading pair
     * @param isBid Boolean indicating if the orderbook to retrieve orders from is an ask orderbook
     * @param orderId The order id to retrieve orders from
     */
    ...
    function _getSpread(address book, bool isBuy) internal view returns (uint32 spreadLimit) {
     ...
     * @param price The price, base/quote regardsless of decimals of the assets in the pair represented with 8 decimals (if 1_000, base is 1_000x quote)
     * @param isBid Boolean indicating if the order is a buy (false) or sell (true) order
     * @param recipient The address of the recipient to receive the traded asset and claim ownership of the made order
     */
    ...
    /**
     * @dev Match bid if `isBid` is true, otherwise match ask if `isBid` is false
     * @param orderbook The address of the orderbook contract for the trading pair
     * @param give The amount of the asset to give
     * @param recipient The address of the recipient to receive the traded asset and claim ownership of the made order
     * @param isBid Boolean indicating if the order is a buy (false) or sell (true) order
     * @param amount The amount of the asset to match
     * @param price The price, base/quote regardless of decimals of the assets in the pair represented with 8 decimals (if 1_000, base is 1_000x quote)
     * @param i The number of orders to match
     * @param n The maximum number of orders to match in the orderbook
     * @return remaining The remaining amount of the asset after the market order has been executed
     * @return k The highest bid price
     */
    function _matchAt(
        address orderbook,
        address give,
     ...
     * @param isBid True if the order is an ask (sell) order, false if the order is a bid (buy) order
     * @param limitPrice The maximum price to match orders at
     ...
     * and if so, makes an order on the orderbook at the market price
     * If an order cannot be made, transfers the remaining asset to either the orderbook or the recipient
     * @param base The address of the base asset for the trading pair
     * @param quote The address of the quote asset for the trading pair
     * @param orderbook The address of the orderbook contract for the trading pair
     * @param remaining The remaining amount of the asset after the market order has been executed
     * @param price The price used to determine if an order can be made at the market price
     * @param isBid Boolean indicating if the order is a buy (false) or sell (true) order
     * @param isMaker Boolean indicating if a order should be made at the market price in orderbook
     * @param recipient The address to receive asset after matching a trade and making an order
     * @return id The unique identifier of the order made at the market price
     */
    function _detMake(
        address base,
        address quote,
     ...
     * @param isBid Whether it is an ask order or a bid order
     ...
     * @param price price of base/quote regardless of decimals of the assets in the pair represented with 8 decimals (if 1_000, base is 1_000x quote)
     * @param amount amount of base or quote asset
     * @param isBid if true, amount is quote asset, otherwise base asset
     * @return converted converted amount from base to quote or vice versa
     * if true, amount is quote asset, otherwise base asset
